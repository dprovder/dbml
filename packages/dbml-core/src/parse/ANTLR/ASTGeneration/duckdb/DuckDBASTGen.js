...
/**
 * DuckDB AST Generator
 * Based on PostgreSQL AST Generator (DuckDB is PostgreSQL-compatible)
 *
 * This implementation reuses PostgreSQL's visitor logic since DuckDB syntax
 * is largely compatible. DuckDB-specific features (STRUCT, MAP, LIST, unsigned types)
 * are handled in the typename and type-related visitor methods.
 */

import { last, flatten, flattenDepth } from 'lodash';
import DuckDBParserVisitor from '../../parsers/duckdb/DuckDBParserVisitor';
import {
...
export default class DuckDBASTGen extends DuckDBParserVisitor {
  constructor () {
    super();
    this.data = {
      schemas: [],
